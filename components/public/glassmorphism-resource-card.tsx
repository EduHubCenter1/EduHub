...
interface GlassmorphismResourceCardProps {
  resource: Resource;
}
...
export function GlassmorphismResourceCard({ resource }: GlassmorphismResourceCardProps) {
  const Icon = getFileIcon(resource.mimeType);
  const { label, className } = typeInfo[resource.type] || typeInfo.other;

  return (
    <div className="relative p-6 rounded-lg shadow-lg backdrop-blur-md bg-white/10 border border-white/20 transition-all duration-300 hover:shadow-xl hover:bg-white/20">
      <div className="flex items-center space-x-4 mb-4">
        ...
        <div className="flex-grow">
          ...
          <p className="text-sm text-gray-200">{label} • {formatFileSize(resource.sizeBytes)} • {resource.fileExt.toUpperCase()}</p>
        </div>
      </div>
